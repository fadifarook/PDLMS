...
class CAENDesktopHighVoltagePowerSupply:
	...
	def ramp_voltage(self, voltage: float, channel: int, device: int = None, ramp_speed_VperSec: float = 5, timeout: float = 10):
		...
		expected_ramping_seconds = ((float(current_voltage[:-1])-float(voltage))**2)**.5/ramp_speed_VperSec
		try:
	...
	def channel_status(self, channel: int, device: int=None):
		...
		status_byte = int(self.query(CMD='MON', PAR='STAT',  CH=channel, BD=device)[-5:-1])
...
class OneCAENChannel:
	...
	def V_mon(self):
		...
		if '+' in channel_polarity:
			polarity = 1
		elif '-' in channel_polarity:
			polarity = -1
		else:
